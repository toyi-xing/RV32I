...
| 文件 | 验证内容 |
|------|----------|
| `control_mix.c` | .data/.bss/.rodata、嵌套循环冒泡排序、函数调用栈、byte/halfword 访存、分支密集路径 |

### 错误码对照

`control_mix.c` 错误码从 1~10 逐步收敛：
...
C 程序永远不直接写 `TEST_STATUS_ADDR(0x00010100)`。（但不是硬件保护，野指针等可能导致被写入）

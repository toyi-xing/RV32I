...
## 5. trap smoke 测试 `手动运行`

| 文件 | 验证内容 |
|------|----------|
| `0501_trap_entry_smoke.S` | ECALL 触发 trap entry，跳转到 mtvec=0x80 的 handler，并由 handler 写 PASS |

`0501_trap_entry_smoke.S` 当前只用于观察 trap entry trace，不放入 `run_all.sh`。运行后日志应出现 `this cycle happen trap_entry`，并由 handler 写 `DMEM_BASE+0x100 = 1` 结束仿真。

...
当前所有汇编测试统一运行 SoC 仿真入口（`sim/soc_asm/`）。旧 core-only 仿真入口已删除，基础 ISA、流水线、trap 和 MMIO 测试都通过同一个 SoC testbench 回归。
...
## 7. interrupt 定向测试 `已通过`

| 文件 | 验证内容 |
|------|----------|
| `0705_interrupt_commit_precise.S` | CSR 写同拍 interrupt 精确提交语义 — pending 已存在时 CSRRS 使能 MIE，中断在 CSR 提交后同拍触发；同时覆盖 CSRW mtvec 同拍 interrupt 使用新 mtvec redirect；验证 mepc、store 提交和 redirect 目标 |
| `0706_mret_interrupt_reentry.S` | MRET 同拍 interrupt 重入 — handler 第一次不清 pending 直接 mret，MRET 边界 MPIE=1 导致立即重入第二次 handler；验证 MRET+interrupt 语义下两次 mcause/mepc 以及 entry/return 的 mstatus.MIE/MPIE |

### 7.1 CSR 写同拍 interrupt（0705）

```asm
csrrs x0, mstatus, MSTATUS_MIE    # 此时 MIE=0, MEIP=1
# 下一拍：CSR 提交后 MIE=1，trap_ctrl 同拍接受 interrupt
after_mie_enable:
```

CSRRS 使能 `mstatus.MIE` 时，硬件在同一 MEM 提交周期完成：
1. CSR 写提交 → `mstatus.MIE = 1`。
2. trap_ctrl 检测 `MIE=1 & MEIE=1 & MEIP=1` → irq_accept 成立。
3. 硬件写 `mepc = mem_interrupt_return_pc_i`（即 `after_mie_enable` 的地址）。
4. Kill IF/ID, ID/EX, EX/MEM；不 kill MEM/WB（CSR 写已提交）。

同一测试还覆盖 `csrw mtvec, xN` 与 interrupt 同拍成立的情况。测试先准备新 handler 地址并制造 GPIO pending，使 `csrw mtvec` 提交边界接受 interrupt；预期 redirect 使用提交后的新 `mtvec`，进入 `alt_trap_handler`，且 `mepc = after_mtvec_write`。

### 7.2 MRET 同拍 interrupt 重入（0706）

第一次 handler 收到 MEIP 后**不清 GPIO pending**，直接 mret：
...
## 8. AXI-Lite/APB 数据子系统集成测试 `已通过`

| 文件 | 验证内容 |
|------|----------|
| `0801_dmem_wait_basic.S` | AXI-Lite DMEM 基本 load/store：SW/LW、SB/LB/LBU、SH/LH/LHU 的 `WSTRB` lane、符号/零扩展和基址+偏移访问 |
| `0802_dmem_wait_forwarding.S` | AXI-Lite DMEM 固定多周期访问下的 forwarding：load 后立即 ALU、store data 转发、load-use stall + MEM wait 和连续 load 链 |
| `0804_mmio_wait_access_fault.S` | GPIO0/UART0/TIMER0 未定义 offset 的 APB `PSLVERR` 经 AXI `SLVERR` 转精确 load/store access fault，并检查 faulting/wrong-path store 无副作用 |
| `0805_wait_interrupt_boundary.S` | AXI-Lite DMEM transaction outstanding 时 interrupt 不越过 older load，response 完成后在提交边界接受 timer interrupt，`mepc == after_wait_load` |

### 8.1 迁移说明

这些文件名沿用 0834 的 `wait` 命名以保持测试编号和回归入口稳定。0836 已删除统一 response-delay wrapper 和对应 mailbox 命令，测试不再配置固定 delay；当前延迟来自 adapter request 注册、AXI-Lite RAM 注册 response 以及 APB SETUP/ACCESS 等真实协议阶段。

### 8.2 当前验证边界

- **DMEM 基本访存**（0801）：验证 AXI-Lite RAM 的 read/write、`WSTRB` byte lane 和 CPU load 截取语义。
- **DMEM forwarding**（0802）：使用正常数据依赖链检查固定多周期 DMEM completion 期间的 MEM backpressure、load-use 和 forwarding。
- **MMIO access fault**（0804）：验证 APB 外设非法 offset 的 `PSLVERR` 经 bridge 返回 `SLVERR`，并在 MEM completion 边界形成精确异常。
- **Interrupt boundary**（0805）：在 older AXI-Lite DMEM load 完成前制造 MTIP，验证 core 不会提前进入 interrupt handler。

当前 SoC testbench 使用固定 readiness 的 AXI-Lite RAM 和固定 `PREADY=1` 外设。这组程序能够证明当前主线集成行为，不覆盖 AW/W 任意先后、各 channel 随机 backpressure 或多周期 `PREADY`。

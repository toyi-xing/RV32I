...
module mmio_uart #(
    ...
    // TX
    input  logic                      tx_ready_i,      // TX backend ready. Tie high in simulation-only use.
    ...
    always_ff @(posedge clk_i or negedge rst_n_i) begin : UART_TX_EVENT
        if (!rst_n_i) begin
            ...
            tx_valid_o <= tx_ready_i & uart_rw[CTRL_IDX][UART_CTRL_TX_EN_BIT] & valid_i & we_i & wo_hit & wo_idx == TXDATA_IDX & be_i[0];
        end
    end
    ...
    assign uart_ro[STATUS_IDX] = ({31'b0, tx_ready_i}                                      << UART_STATUS_TX_READY_BIT) |

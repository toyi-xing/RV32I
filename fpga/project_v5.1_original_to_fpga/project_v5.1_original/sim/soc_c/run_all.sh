...
# SoC C regression:
# discover all sw/c/*.c tests, build IMEM/DMEM images, build tb_rv32i_soc once,
...
if [ "${#TESTS[@]}" -eq 0 ]; then
    echo "ERROR: no C tests found in ${REPO_ROOT}/sw/c" >&2
    exit 1
fi
...
echo ">>> [1/3] Building all SoC C IMEM/DMEM files..."
for t in "${TESTS[@]}"; do

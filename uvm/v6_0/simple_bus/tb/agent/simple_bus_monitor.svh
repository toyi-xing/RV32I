//------------------------------------------------------------------------------
// 文件      : uvm/v6_0/simple_bus/tb/agent/simple_bus_monitor.svh
...
class simple_bus_monitor extends uvm_component;
    ...
    task run_phase(uvm_phase phase);
        ...
        int unsigned resp_cycle;    // 记录 resp 的拍数
        // rst 行为
        cycle_cnt  = 0;
        pending    = 1'b0;
        ...
        tr         = simple_bus_item::type_id::create("", this); // 创建一个 transfer 对象给 tr 语柄，供第一个 transaction 使用
        resp_cycle = cycle_cnt;
        // rst 后每拍监控 dut 端口情况
        forever begin
            @(vif.mon_cb);
            cycle_cnt++;
            if(vif.rst_n_i !== 1'b1) begin  // 测试过程中再次复位，这里复位不清 cycle_cut
                pending    = 1'b0;
                tr         = simple_bus_item::type_id::create("", this);
                resp_cycle = cycle_cnt;
                continue;
            end
            // 利用 item.idle_cycles sentinel 值，检测 req.valid 首拍，记录 idle_cycles
            if (vif.mon_cb.req_i.valid && !pending && tr.idle_cycles == -1) begin
                tr.idle_cycles = cycle_cnt - resp_cycle - 1; // resp 和下一个 req 必然不能同拍
            end
            // 检测 req 握手
            if (vif.mon_cb.req_i.valid && vif.mon_cb.req_ready_o) begin
                ...
                pending       = 1'b1;
                tr.write      = vif.mon_cb.req_i.write;
                tr.be         = vif.mon_cb.req_i.be;
                tr.addr       = vif.mon_cb.req_i.addr;
                tr.wdata      = vif.mon_cb.req_i.wdata;
                tr.resp_delay = 0;
            end
            // 记录 resp_delay

# v6.0 Data Subsystem UVM 仿真流程

本文定义 `uvm/v6_0/data_subsystem` 的 VCS/UVM 仿真入口、产物和结果判定。该环境绑定 v6.0 `data_subsystem` DUT 快照，其 core-side 接口采用 single-outstanding simple data bus；主线 RTL 后续演进不改变本环境的 DUT 语义。

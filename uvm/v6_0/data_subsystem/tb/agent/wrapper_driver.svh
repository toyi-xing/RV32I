//------------------------------------------------------------------------------
// 文件      : uvm/v6_0/data_subsystem/tb/agent/wrapper_driver.svh
...
class wrapper_driver extends uvm_driver #(wrapper_item);
    ...
    protected function int item_check(wrapper_item item);
        case (item.target)
    ...
    protected task automatic drive_item(wrapper_item item);
        vif.set_target_resp_delay(item.target, item.delay_cycles);
        `uvm_info(get_type_name(), {"wrapper driver applied configuration:", item.item2string("applied cfg item")}, UVM_MEDIUM)
    endtask

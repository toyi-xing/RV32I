...
- **MMIO 地址图及外设**：GPIO0（OUT/IN/OE）、UART0（TXDATA/STATUS/CTRL），地址译码集成于 data_subsystem，地址图规划在 `soc_pkg.sv` 中，且已在 `sw/include/platform.h` 中针对 c 程序需要进行声明。
...
rtl/periph/          # MMIO 外设（mmio_gpio、mmio_uart）
...
- `tb/sv/tb_core_pipeline5.sv`：当前仍然兼容的旧仅测试 CPU 核的测试平台，预计后续 CPU 核添加异步中断功能必须配合 MMIO 外设时将删除本测试平台。

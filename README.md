...
**汇编测试程序：** (sw/asm/) `0001_smoke.S`、`0101_branch.S`、`0102_alu_imm.S`、`0103_alu_reg.S`、`0104_load_store.S`、`0105_jump.S`、`0106_u_type.S`，须搭配 asm 链接程序 `sw/linker/asm_test.ld`

**c测试程序：** (sw/c) `0201_c_smoke.c`、`0202_dmem_init.c`，须搭配 c 启动程序 `sw/c_runtime/crt0.S` 和 c 链接程序 `sw/linker/c_baremetal.ld`
...
单测试脚本参数支持四位编号或完整 basename，例如 `0102` 和 `0102_alu_imm` 等价。
...
```bash
sim/single_cycle_asm/run_test.sh <四位编号或完整basename>
```

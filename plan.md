# v3.0 最小 memory map 与 MMIO 执行计划 `已完成`
...
### 8.1 保留现有 pipeline5 脚本 `已完成`

`sim/pipeline5_asm` 和 `sim/pipeline5_c` 继续作为 core-level 回归入口。

`sim/pipeline5_c/run_all.sh` 已补齐，作为 core-only C 回归入口。它自动枚举 `sw/c/` 下旧 core 平台适用的 C 测试（当前为 `0000` 到 `0599` 编号段），不跑 `06xx` 之后依赖 SoC/MMIO 的程序。
...
### 8.2 新增 `sim/soc_asm/run_test.sh` `已完成`
...
当前实现拆成三段，便于单独构建或单独运行：

```text
sim/soc_asm/05_build_mem.sh
sim/soc_asm/06_run_sim.sh
sim/soc_asm/run_test.sh
```

### 8.3 新增 `sim/soc_c/run_test.sh` `已完成`
...
```text
sim/soc_c/05_build_mem.sh
sim/soc_c/06_run_sim.sh
sim/soc_c/run_test.sh
```

### 8.4 新增 SoC 分组 `run_all.sh` `已完成`

SoC 平台天然包含 core、IMEM、DMEM、trap 和 MMIO，因此 SoC 分组 `run_all.sh` 可以直接作为当前源码目录的全量平台回归入口。
...
当前 `sim/soc_asm/run_all.sh` 自动枚举 `sw/asm/*.S`，因此会同时回归旧基础 ASM、流水线 ASM、trap ASM 和 SoC ASM。当前 `sim/soc_c/run_all.sh` 自动枚举 `sw/c/*.c`，因此会同时回归旧 C 程序和 SoC C 程序。

如果后续出现“暂时不能进入总回归”的实验程序，再单独引入目录分组或编号过滤；在当前阶段，SoC all 直接跑全部已有源码最省维护成本。

## 9. SoC directed test 规划 `已完成`
...
| 编号 | 建议文件 | 目标 |
|---:|---|---|
| `0601` | `sw/asm/0601_soc_smoke.S` | 旧主路径程序通过 `rv32i_soc` 跑通，证明 SoC wrapper 不破坏 core；已通过 `sim/soc_asm/run_test.sh 0601` |
| `0602` | `sw/asm/0602_uart_tx.S` | 软件开启 UART，写 `TXDATA`，testbench 观察 TX event；已通过 `sim/soc_asm/run_test.sh 0602` |
| `0603` | `sw/asm/0603_gpio_rw.S` | 写 `GPIO_OUT/OE`，读 `GPIO_IN`，验证普通 RW/RO MMIO；已通过 `sim/soc_asm/run_test.sh 0603` |
| `0604` | `sw/asm/0604_mmio_access_fault.S` | 访问未实现 MMIO/保留区，检查 load/store access fault；已通过 `sim/soc_asm/run_test.sh 0604` |
| `0605` | `sw/asm/0605_mmio_misaligned_priority.S` | MMIO 不对齐访问优先报 misaligned，不报 access fault；已通过 `sim/soc_asm/run_test.sh 0605` |
| `0606` | `sw/asm/0606_wrong_path_mmio.S` | branch/trap wrong-path store 不应触发 UART/GPIO 副作用；已通过 `sim/soc_asm/run_test.sh 0606` |
| `0651` | `sw/c/0651_soc_mmio_smoke.c` | C 程序通过 `platform.h` 访问 UART/GPIO，做 MMIO 冒烟；已通过 `sim/soc_c/run_test.sh 0651` |
...
- 当前已通过 `sim/soc_asm/run_all.sh`：19 passed, 0 failed。
- 当前已通过 `sim/soc_c/run_all.sh`：5 passed, 0 failed。
- 当前已通过 `sim/pipeline5_c/run_all.sh`：4 passed, 0 failed。

### 9.2 0833 interrupt/timer 测试预留 `无需改动`
...
### 9.3 0834 wait-state/backpressure 测试预留 `无需改动`
...
### 10.1 新增 C 侧 platform header `已完成`
...
### 11.1 同步 `sw/linker/readme.md` `已完成`
...
### 11.2 同步 README 和 simulation flow 文档 `无需改动`
...
- `docs/simulation_flow_asm.md`
  - 说明该流程仍用于 core-level regression。
- `docs/simulation_flow_c.md`
  - 说明该流程仍用于 core-level regression。
...
| `sim/soc_asm/05_build_mem.sh` | SoC ASM 测试镜像构建入口 |
| `sim/soc_asm/06_run_sim.sh` | SoC ASM 测试仿真入口 |
| `sim/soc_asm/run_test.sh` | SoC ASM 定向测试入口 |
| `sim/soc_asm/run_all.sh` | SoC ASM 分组回归入口 |
| `sim/soc_c/05_build_mem.sh` | SoC C 测试镜像构建入口 |
| `sim/soc_c/06_run_sim.sh` | SoC C 测试仿真入口 |
| `sim/soc_c/run_test.sh` | SoC C 定向测试入口 |
| `sim/soc_c/run_all.sh` | SoC C 分组回归入口 |
| `sim/pipeline5_c/run_all.sh` | core-only C 分组回归入口 |
| `sw/include/platform.h` | C 侧 MMIO 地址常量和访问函数 |
| `sw/asm/0601_soc_smoke.S` | SoC wrapper ASM 冒烟 |
| `sw/asm/0602_uart_tx.S` | UART0 TX MMIO ASM 测试 |
| `sw/asm/0603_gpio_rw.S` | GPIO0 RW/RO MMIO ASM 测试 |
| `sw/asm/0604_mmio_access_fault.S` | 未映射 MMIO access fault ASM 测试 |
| `sw/asm/0605_mmio_misaligned_priority.S` | MMIO misaligned 优先级 ASM 测试 |
| `sw/asm/0606_wrong_path_mmio.S` | wrong-path MMIO 副作用门控 ASM 测试 |
| `sw/c/0651_soc_mmio_smoke.c` | SoC MMIO C 冒烟 |
...
| `docs/simulation_flow_asm.md` | 说明该流程仍是 core-level regression，SoC 测试走 `sim/soc_asm` |
| `docs/simulation_flow_c.md` | 说明该流程仍是 core-level regression，SoC 测试走 `sim/soc_c` |

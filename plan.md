...
- 支持 UART0 TX MMIO 寄存器，软件写寄存器后 testbench 能观察字符。
...
本阶段只实现 UART0/GPIO0。`TIMER0/ACCEL0` 是地址预留；在对应模块未实现前，访问这些窗口应视为未映射地址并产生 access fault。
...
### 2.1 新建 `rtl/periph/mmio_uart_tx.sv`

模块职责：

- 提供 UART0 最小 MMIO 寄存器块。
- 软件写 `TXDATA` 时输出一拍 `tx_valid_o` 和 `tx_data_o`。
- `STATUS.tx_ready` 固定为 1。
- `CTRL.enable` 是 RW 寄存器，复位后为 1。
...
- `addr_i` 建议传完整 byte address，模块内部用 `addr_i - UART0_BASE` 得到 offset。
...
| offset | 名称 | 属性 | 行为 |
|---:|---|---|---|
| `0x00` | `TXDATA` | WO/RW | 写入时 `tx_valid_o` 拉高一拍，`tx_data_o = wdata_i[7:0]` |
| `0x04` | `STATUS` | RO | bit0 固定为 1，表示 ready |
| `0x08` | `CTRL` | RW | bit0 enable，复位为 1 |

写行为：

- `valid_i && we_i && offset == TXDATA` 时，如果 `ctrl_enable == 1`，输出字符事件。
- `TXDATA` 保存最近一次写入的低 8 bit，便于读回 debug。
- 写 `STATUS` 忽略。
...
### 2.2 新建 `rtl/periph/mmio_gpio.sv`
...
## 3. 新增 `rtl/soc/data_subsystem.sv`
...
- 实例化 `simple_ram`、`mmio_uart_tx`、`mmio_gpio`。
...
| 外设 | base | 当前状态 |
|---|---:|---|
| UART0 | `0x0008_0000` | 已实现 TXDATA/STATUS/CTRL |
| GPIO0 | `0x0008_0100` | 已实现 OUT/IN/OE |
| TIMER0 | `0x0008_0200` | 地址预留，访问 fault |
| ACCEL0 | `0x0008_1000` | 地址预留，访问 fault |
...
| 文件 | 作用 |
|---|---|
| `rtl/periph/mmio_uart_tx.sv` | UART0 最小 MMIO 寄存器块 |

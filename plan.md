...
req_gpio0_valid/req_uart0_valid/req_timer0_valid 当拍：
  ...
  - 根据 mmio_resp_delay_cycles_i 决定同拍 response 或延迟 response。
```
...
### 8.2 固定采用 wrapper 路线：外设本体仍是固定响应 register block
...
0834 第一版可以不改 `simple_ram`，仍由 testbench 实例化；新增或内嵌 `dmem_simple_slave` 连接外置 DMEM 端口，并由该 slave 包装 response 时序。

### 11.2 增加 DMEM simple slave wrapper
...
若为了减少文件数量，也可以先把 `dmem_simple_slave` 作为 `data_subsystem.sv` 内部局部逻辑实现，但计划口径仍按独立 target slave 边界理解，便于后续拆出模块。

### 11.3 DMEM write 副作用时机

如果 DMEM store 在 `dmem_simple_slave` request accepted 当拍写入，但 response 延迟返回，软件可见提交在 response OK。
...
- `rtl/mem/simple_data_ram_slave.sv`（若新增独立文件）
- 外设 simple slave wrapper（若新增独立文件）
...
- SoC/data_subsystem 透出由 TB 驱动的 delay 配置输入，默认值为 0，并分发到 DMEM/MMIO target slave/wrapper。
- TB 可通过 plusarg 设置 `dmem_resp_delay_cycles/mmio_resp_delay_cycles` 初始值。
- TB mailbox 新增或扩展命令，软件写 mailbox 后，TB 动态更新 DMEM/MMIO delay 配置。
- delay 新值只影响后续新 accepted request；已经 pending 的 response 使用 target slave `req_fire` 当拍锁存的 delay/counter，不受后续修改影响。
- delay 配置先固定为小位宽无符号值，例如 8 bit；软件测试只使用较小周期数，避免 timeout。

推荐 mailbox 命令语义：

```text
set_dmem_delay(cycles)
set_mmio_delay(cycles)
set_data_delay(dmem_cycles, mmio_cycles)
```

这些命令只属于 TB 验证协议，不属于 SoC 软件 ABI，也不写入 `rtl/periph/readme.md` 的通用外设手册。

本阶段不要求完整随机延迟，只要求 RTL/TB 具备 directed test 可控插入等待的入口。随机/组合 wait-state 应放到 0835 的验证环境中展开。

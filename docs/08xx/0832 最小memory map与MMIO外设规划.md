...
| 最小 MMIO 外设 | 支持 UART TX 和 GPIO 两类最小外设 |
...
64 KiB 的 MMIO window 对当前教学 SoC 足够宽。后续如果接多个外设或 accelerator，也可以继续在 `0x0009_0000` 之后扩展，或者把 MMIO window 放大。
...
| 外设 | 建议 base | 建议大小 | 作用 |
|---|---:|---:|---|
| UART0 | `0x0008_0000` | `0x100` | 软件写 TXDATA，testbench 打印字符 |
| GPIO0 | `0x0008_0100` | `0x100` | 软件写 OUT、读 IN，用于可观察控制寄存器 |
| TIMER0 | `0x0008_0200` | `0x100` | 本步可只保留地址，后续 interrupt 阶段实现 |
| ACCEL0 | `0x0008_1000` | `0x1000` | 可预留给后续 TPU/NPU 或 MAC accelerator |
...
UART0 第一版不需要真实串口波特率发送器，只需要提供一个 MMIO 寄存器，让 testbench 能观察字符输出。

建议寄存器：

| offset | 名称 | 属性 | bit 定义 | 行为 |
|---:|---|---|---|---|
| `0x00` | `TXDATA` | WO 或 RW | `[7:0] data` | 软件写入低 8 bit，testbench 打印对应字符 |
| `0x04` | `STATUS` | RO | `bit0 tx_ready` | 第一版固定为 1，表示总能接收下一个字符 |
| `0x08` | `CTRL` | RW | `bit0 enable` | 第一版实现为普通 RW；复位后默认启用 |

软件最小使用路径只依赖 `TXDATA` 和 `STATUS`，但 RTL 寄存器块同时提供 `CTRL`，方便后续真实 UART 或 interrupt enable 扩展：

```c
#define UART0_BASE   0x00080000u
...
UART TX 的仿真效果最直观，用来证明 MMIO store 能触发外设动作；GPIO 更适合验证普通 RW/RO 寄存器、byte enable 和状态保持。两者合在一起能覆盖最小 MMIO 平台最常见的两类外设语义。
...
当前 `core_pipeline5.sv` 对外暴露一组 dmem 风格接口：
...
建议保持 `core_pipeline5.sv` 是 CPU core，不在里面实例化 UART/GPIO。更推荐新增一个 SoC 或 platform wrapper：

```text
rtl/core/core_pipeline5.sv       只放 CPU core
...
rtl/periph/mmio_uart_tx.sv       UART TX 寄存器块
...
当前 core 已经为 store 生成 `dmem_be_o`。MMIO 外设应明确是否支持 byte/halfword 写。
...
当前 `mem_stage` 已经用 `valid_i`、`exception_valid_i` 和 misaligned 检测门控 `dmem_we_o`。加入 MMIO 后仍应保持同一原则：
...
因此，对 CPU core 来说，MMIO load 不需要新的写回来源。它仍然是 `WB_MEM`，只是 `dmem_rdata_i` 的来源由 RAM 改成了 MMIO 寄存器块。
...
| `rtl/periph/mmio_uart_tx.sv` | 最小 UART TX MMIO 寄存器块，写 TXDATA 时输出字符事件 |
...
| `UART0_BASE/UART0_SIZE_BYTES` | UART0 寄存器窗口 |
| `GPIO0_BASE/GPIO0_SIZE_BYTES` | GPIO0 寄存器窗口 |
...
| 文件 | 修改方向 |
|---|---|
| `rtl/core/core_pipeline5.sv` | 新增数据访问错误输入并传给 `mem_stage` |
...
本阶段要求未映射地址进入 trap，因此 `core_pipeline5.sv` 和 `mem_stage.sv` 需要接收数据子系统给出的 access fault 信息。这样后续加入 timer、accelerator 或 ready/valid 时，不需要重新设计未映射访问的架构行为。

...
```text
写 MTIME          -> 本拍 MTIME 保存软件写入值，不再自增。
写 MTIMECMP/CTRL -> 不阻止 MTIME 自增；是否自增取决于时钟沿前的 CTRL.enable。
```

也就是说，软件写 `CTRL.enable=1` 后从下一拍开始计数；软件写 `MTIMECMP` 或其它非 `MTIME` 寄存器时，不会人为暂停已经运行的计数器。这个口径让 directed test 可以精确设置 `MTIME` 初值，同时避免普通配置写导致 timer 漏 tick。
...
`gpio_in_i` 通常来自 SoC 外部或 testbench 驱动，不能假设与 core `clk_i` 同步。本阶段 GPIO 外设内部先做两级同步，再用同步后的值作为 `IN` 读值和 interrupt 触发检测输入。这样会让软件看到的输入和中断检测相对外部真实变化延迟约两拍，但避免直接把异步输入送进边沿检测逻辑。

触发检测可以描述为：

```text
gpio_in_meta <= gpio_in_i
gpio_in_curr <= gpio_in_meta
gpio_in_prev <= gpio_in_curr
...
`OE` 本阶段只表示 CPU 侧输出使能，不参与 interrupt mask。`mmio_gpio` 只对同步后的 `gpio_in_i` 视图做触发检测；如果后续引入真实双向 pad 或 FPGA 引脚 wrapper，应由外层决定 `gpio_in_i` 的来源和输出回读策略。

...
### 3.2 流水线测试不使用 branch/JAL/JALR

control hazard flush 尚未实现。branch/JAL/JALR 在 EX 产生 redirect 后，IF/ID 和 ID/EX 中的错误路径指令不会被清除，会干扰测试。所有流水线测试均为纯顺序流。
...
- 如果看到非法指令提交，先检查是不是 redirect flush 缺失导致错误路径指令进入了流水线（当前阶段应避免使用 branch/JAL/JALR）。

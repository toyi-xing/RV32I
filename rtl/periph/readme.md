...
`mmio_timer32` 是 32-bit 教学用 timer 外设。它提供 `MTIME` 计数器、`MTIMECMP` 比较值、`CTRL` 使能和 `STATUS` 观察口。`mtip_o` 是 level pending，不是 pulse。
...
| bit | 名称 | 说明 |
|---:|---|---|
| 0 | `enable` | 1 时允许 `MTIME` 自增，并允许比较结果输出到 `mtip_o`。 |
...
timer interrupt 输出 `mtip_o` 与 `STATUS.mtip` 同义，是 level 信号。只要 `CTRL.enable=1` 且 `MTIME >= MTIMECMP`，它就保持为 1；条件不成立时自动为 0。软件清 timer interrupt 的方式不是 acknowledge pending flop，而是让比较条件不成立，例如把 `MTIMECMP` 写到未来值，或关闭 `CTRL.enable`。
...
TIMER32 的中断是连续比较结果，不是一闪而过的事件。条件成立则 `mtip_o` 保持高，不成立则自动恢复低。软件清中断的方式是改变比较条件，不需要 acknowledge pending 寄存器。
